...
try:
    ...
    base_dir_root = settings.BASE_DIR
except:
    from pnerf import pnerf as pnerf
    import pathlib
...
class H5PytorchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        print("TERTIARY SHAPE", np.shape(tertiary))
        print("PRIMARY STRING", prim)
        print("READING ITEM\n", "tertiary[", index, "][", int(mask.sum()), "]")
        # time.sleep(15000)
        # for e in tertiary:
        #    for n in e:
        #        print(n)
...
def write_out(*args, end='\n'):
    ...
    if globals().get("experiment_id") is not None:
        with open(base_dir_root + "/protAPI/proteinnet/output/" + globals().get("experiment_id") + ".txt", "a+") as output_file:
...
def write_model_to_disk(model):
    path = base_dir_root + "/protAPI/proteinnet/output/models/" + globals().get("experiment_id") + ".model"
...
def write_result_summary(accuracy):
    ...
    with open(base_dir_root + "/protAPI/proteinnet/output/result_summary.txt", "a+") as output_file:
        output_file.write(output_string)
        output_file.flush()
    print(output_string, end="")
...
def calculate_dihedral_angles_over_minibatch(atomic_coords_padded, batch_sizes, use_gpu):
    ...
    print("shape before:", atomic_coords_padded.shape)
    print("shape after:", atomic_coords.shape)
    print("atomic coords padded fo coompute da:", atomic_coords)
    print("BATCH SIZES", len(batch_sizes))
    print("BATCH KIND", type(batch_sizes))
    for idx, _ in enumerate(batch_sizes):
        print("TYPE", type(atomic_coords))
        print("SHAPE", np.shape(atomic_coords))
        print("Time iterator:", idx, batch_sizes[idx])
        # input('...')
        # print("[CALCULANDO ANGULOS]",atomic_coords[idx][:batch_sizes[idx]], "\n[FIN CALCULO]")
        # time.sleep(15000)
        print("dihidral calculados en atomicoords:", idx, batch_sizes[idx])
        print("las cordenadas atomicas del calculado", atomic_coords[idx][:batch_sizes[idx]])
        # input('...')
        angles.append(calculate_dihedral_angles(atomic_coords[idx][:batch_sizes[idx]], use_gpu))  # de 0 a 201
    for a in angles:
        print("ANGLE", a)
...
def calculate_dihedral_angles(atomic_coords, use_gpu):
    print("atomic_coords recibido", atomic_coords.shape)
...
def pass_messages(aa_features, message_transformation, use_gpu):
    ...
    aa_messages = torch.stack((features_repeated.transpose(0, 1), features_repeated)).transpose(0, 1).transpose(1,
                                                                                                                2).view(
        -1, 2, feature_size)
    aa_msg_pairs = torch.masked_select(aa_messages, eye_inverted).view(-1, 2,
                                                                       feature_size)  # (aa_count^2 - aa_count) x 2 x aa_features     (all pairs except for reflexive connections)
