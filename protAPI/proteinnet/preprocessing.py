...
import h5py
try:
    from protAPI.proteinnet.util import AA_ID_DICT, calculate_dihedral_angles, protein_id_to_str, get_structure_from_angles, \
    ...
    from util import AA_ID_DICT, calculate_dihedral_angles, protein_id_to_str, \
...
def process_raw_data(use_gpu, force_pre_processing_overwrite=True):
    ...
    input_files = glob.glob(base_dir + "data/raw/*")
    print(input_files)
    ...
    for file_path in input_files_filtered:
        if platform.system() is 'Windows':
            ...
            filename = file_path.split('/')[-1]
        preprocessed_file_name = base_dir + "data/preprocessed/" + filename + ".hdf5"
...
def process_file(input_file, output_file, use_gpu):
    ...
    input_file_pointer = open(base_dir + "data/raw/" + input_file, "r")
    ...
    while True:
        ...
        pos = torch.masked_select(torch.Tensor(tertiary_padded), mask).view(9, -1).transpose(0, 1).unsqueeze(
            1) / 100  # wtf
...
process_file('sample.txt', 'sample.h5', False)